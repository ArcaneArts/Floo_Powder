...
/**
 * Fabric 1.18.2 implementation using PoseStack and RenderSystem.
 */
public class FlooGraphics extends GuiComponent implements IFlooGraphics {
  ...
  public void blit(ResourceLocation texture, int x, int y, float u, float v, int width, int height, int texWidth, int texHeight) {
    RenderSystem.setShaderTexture(0, texture);
    blit(poseStack, x, y, (int) u, (int) v, width, height, texWidth, texHeight);
  }
  ...
  public void fillGradient(int x1, int y1, int x2, int y2, int colorTop, int colorBottom) {
    fillGradient(poseStack, x1, y1, x2, y2, colorTop, colorBottom);
  }
